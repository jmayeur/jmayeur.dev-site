    ...
    <div className={`relative isolate min-h-screen overflow-x-clip ${bgClass}`}>
      ...
      <header className="mx-auto w-full max-w-5xl px-6 pb-10 pt-12 md:px-10 md:pt-16">
        ...
        <h1 className="mt-3 max-w-3xl text-balance text-4xl font-semibold leading-tight md:text-6xl">
          Observability <span className={`text-2xl md:text-4xl font-normal ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>(O11y)</span>
        </h1>
        <p className={`mt-5 max-w-2xl text-pretty text-base leading-relaxed md:text-lg ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>
          Observability only creates value when the team can act on the signal.
        </p>

        <nav aria-label="Page sections" className="mt-8 flex flex-wrap gap-2 sm:gap-3">
          <a className="chip-link" href="#philosophy">
            Philosophy
          </a>
        </nav>
      </header>
      ...
      <main id="content" className="mx-auto grid w-full max-w-5xl gap-8 px-6 pb-16 md:px-10">
        <section id="philosophy" aria-labelledby="philosophy-heading" className="panel">
          <h2 id="philosophy-heading" className="panel-title">
            Philosophy
          </h2>
          <p className="panel-text">
            Observability is a trap, or it can be. It's too easy to focus on the tooling, and forget the purpose.
            Throw NewRelic or Splunk, OTEL, DataDog, Dynatrace or whatever platform you chose and log everything.
            Don't worry, the bill doesn't come due until later. A key lesson that I keep learning is that a signal is
            only useful if I can act on it.
          </p>
          <p className="panel-text mt-4">
            When I think about how to consider O11y for a project, especially in Agentic O11y, I begin with what can
            I do with the insight. Take logging network call failures. Of course if a service is failing to reach a
            downstream system, or a user experience is failing to post data to some endpoint, we need to know so we
            can address the issue. Unfortunately there's always noise. Safari is especially hard on analytics and
            tracking services.
          </p>
          <p className="panel-text mt-4">
            When I think about a problem like sorting signal from noise, I start with what would I expect to happen
            if I see a 2% spike in signal, what about 0.01%. The primary driving mindset is to think of O11y more
            like GPS than a smoke alarm. I want it to tell me how to keep on the right path, not cause panic. The
            worst thing that can happen to a team is when they have over-indexed on capturing every bit of data, and
            become inured to the sound of a PagerDuty alarm bell.
          </p>
          <p className="panel-text mt-4">
            Fundamentally, O11y is about surfacing the insights from within the black boxes of complex multi-domain
            systems, but with the intent of keeping the lights on, not raising alarms.
          </p>
        </section>
      </main>
    </div>
